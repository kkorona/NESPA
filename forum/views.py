...
from forum.models import Post, Comment, Attach, urllib.parse
...
class PostDV(FormMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if f'username/forum/{post_id}' not in self.request.session:
            current_post = Post.objects.get(pk=post_id)
            current_post.update_counter
            self.request.session[f'username/forum/{post_id}'] = True
...
def edit(request, article_id):
    ...
    if request.method == "POST":
        ...
        for file in files:
            ...
            destination_path = urllib.parse.unquote(os.path.join(settings.BASE_DIR, 'media','attached','forum',str(article.id)))
