...
from error_reports.forms import CommentForm
...
class PostDV(FormMixin, DetailView):
    ...
    def get_success_url(self, **kwargs):
        return reverse('error_reports:post_detail', kwargs = {'pk': self.object.pk })
        
    def get_context_data(self, **kwargs):
        context = super(PostDV, self).get_context_data(**kwargs)

        post_id = self.kwargs['pk']
        if f'username/error_reports/{post_id}' not in self.request.session:
            current_post = Post.objects.get(pk=post_id)
            current_post.update_counter
            self.request.session[f'username/error_reports/{post_id}'] = True
...
def deleteComment(request, article_id, comment_id):
    ...
    comment.save()
    return redirect('/error_reports/post/' + str(article_id))
...
def edit(request, article_id):
    ...
    if request.method == "GET":
        ...
        return render(request, 'error_reports/edit.html',{'article_content' : content, 'article_title' : article.title, 'attachments' : article.attach_set.all()})
        
def write(request):
    if request.method == "GET":
        return render(request, 'error_reports/write.html')
    if request.method == "POST":
        username = request.session['username']
        usertype = request.session['usertype']
        userid = request.session['userid']
        ...
        for file in files:
            ...
            destination_path = urllib.parse.unquote(os.path.join(settings.BASE_DIR, 'media','attached','error_reports',str(article.id)))
            if not os.path.exists(destination_path):
                os.makedirs(destination_path)
            destination_path = os.path.join(destination_path, fname)
            shutil.move(departure_path,destination_path)
        ...
        return redirect('error_reports:post_list')
