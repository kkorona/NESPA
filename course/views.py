...
class PostDV(FormMixin, DetailView):
    ...
    def get_success_url(self, **kwargs):
        return reverse('course:post_detail', kwargs = {'pk': self.object.pk })
        
    def get_context_data(self, **kwargs):
        context = super(PostDV, self).get_context_data(**kwargs)

        post_id = self.kwargs['pk']
        if f'username/course/{post_id}' not in self.request.session:
            current_post = Post.objects.get(pk=post_id)
            current_post.update_counter
            self.request.session[f'username/course/{post_id}'] = True
...
def deleteComment(request, article_id, comment_id):
    ...
    comment.save()
    return redirect('/course/post/' + str(article_id))
...
def edit(request, article_id):
    ...
    if request.method == "POST":
        ...
        for file in files:
            ...
            destination_path = urllib.parse.unquote(os.path.join(settings.BASE_DIR, 'media','attached','course',str(article.id)))
    ...
    if request.method == "GET":
        ...
        return render(request, 'course/edit.html',{'article_content' : content, 'article_title' : article.title, 'attachments' : article.attach_set.all()})
        
def write(request):
    if request.method == "GET":
        return render(request, 'course/write.html')
    if request.method == "POST":
        username = request.session['username']
        usertype = request.session['usertype']
        userid = request.session['userid']
        ...
        return redirect('course:post_list')
